...
var router = function (config) {
 ...
 // connect
  appRouter.post('/connect',function(req,res){
     ...
     var dbconfig = new DbConfig(req.body);
     var session = req.session;

     host(dbconfig).infos(function(err,result){
       if(!err){
           if(!session.hosts){
             session.hosts = [];
           }
           session.hosts.push({sort: session.hosts.length , config:dbconfig});
       }
       res.json({
     ...
     //res.json({dbs:mongo.databases,colls:mongo.collections,gridFSBuckets: utils.colsToGrid(mongo.collections)});
  });
  ...
  appRouter.get('/', function(req,res){
    ...
    if(req.session.hosts){
      var funcs = [];
      /*此处使用promise并发获取各个主机信息时结果会重复  待解决*/
      //req.session.hosts.forEach(function(item){
      //  var dbhost = host(item.config);
      //  funcs.push(new Promise(function (resolve, reject) {
      //    dbhost.infos(function(err,result){
      //      if(err){reject(err);}
  ...
  appRouter.get('/host/:host/db/:db/table/:table', routes().getDatas);
  appRouter.delete('/host/:host/db/:db/table/:table', routes().delete);
  appRouter.post('/host/:host/db/:db/table/:table', routes().update);
